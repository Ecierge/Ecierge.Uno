...
{
    public LocationBreadcrumbLayout(LocationBreadcrumbBar breadcrumb)
    {
#if HAS_UNO
        m_breadcrumb = WeakReferencePool.RentWeakReference(this, breadcrumb);
#else
        m_breadcrumb = new WeakReference<LocationBreadcrumbBar>(breadcrumb);
#endif
    }
    ...
    protected override Size MeasureOverride(NonVirtualizingLayoutContext context, Size availableSize)
    {
        ...
        {
            var breadcrumbItem = (LocationBreadcrumbBarItem)GetElementAt(context, i);
        ...
        {
            if (GetElementAt(context, 0) is LocationBreadcrumbBarItem ellipsisButton)
            {
...
#if HAS_UNO
        var breadcrumb = m_breadcrumb.Target as LocationBreadcrumbBar;
        if (breadcrumb != null)
        {
            breadcrumb.ReIndexVisibleElementsForAccessibility();
        }
#else
        if (m_breadcrumb.TryGetTarget(out var breadcrumb))
        {
            breadcrumb.ReIndexVisibleElementsForAccessibility();
        }
#endif
